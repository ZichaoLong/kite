...
架构蓝本是姊妹项目 **FOCUS**（飞书 ↔ Codex app-server 桥接）：复用其分层架构、状态契约与工程纪律，把上游适配层从 `codex app-server` 换成 kimi-code 的 **kap-server**(REST + WebSocket)。

产品取向**克制**：功能增速不得超过架构承载力。每个功能进入开发前必须先立合同（见 `docs/contracts/mvp-scope.md` 的承载力门槛）；宁可少做、做透，不做多、做漏。
...
`architecture/`、`decisions/` 为 active repository facts。daemon 化部署已完成
（systemd --user `kite.service`,autostart 已启用；管理面 `kitectl service`)。
下一步：Phase 2 候选评审（群聊、volatile 流式卡、图片、允许名单）。
...
| --- | --- |
| FOCUS | 架构蓝本与资产复用来源 |
| kimi-code | 上游；kap-server 行为以其仓库的实际代码为 source of truth |
