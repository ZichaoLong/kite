...
backoff, graceful shutdown), owns the WS subscription client, and publishes a
best-effort runtime status for kitectl. On SIGTERM/SIGINT it shuts the managed
child down cleanly.
"""
    ...
    try:
        while not stop_event.is_set():
            ...
            ws = KapWsClient(
                ...
                on_event=on_event,
                on_resync_required=on_resync,
                on_connection_change=on_connection_change,
            )
            ws.start()
            for binding in binding_store.load_all().values():
                ws.subscribe(binding["session_id"])
